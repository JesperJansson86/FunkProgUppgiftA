...
import static Inlämningsupgift.Category.FOOD;
...
public class RewriteMe {
    ...
    public int getAmountOfQuestionsForACertainCategory(Category category) {
        ...
        return (int) questions.stream().filter(e -> e.category.equals(category)).count();

    }

    //Skapa en lista innehållandes samtliga frågesträngar i databasen
    public List<String> getListOfAllQuestions() {
        return questions.stream().map(e -> e.question).collect(Collectors.toList());
    ...
    public List<String> getAllQuestionStringsBelongingACategory(Category category) {
        return questions.stream().filter(e -> e.category.equals(category)).map(e -> e.question).collect(Collectors.toList());
    ...
    public List<String> getAllAnswerOptionsDistinct() {
        return questions.stream().flatMap(e -> e.answers.stream()).distinct().collect(Collectors.toList());
    ...
    public boolean isThisAnAnswerOption(String answerCandidate) {
        return questions.stream().flatMap(e -> e.answers.stream()).anyMatch(d -> d.equals(answerCandidate));
    }
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate) {
        ...
        return (int) questions.stream().flatMap(e -> e.answers.stream()).filter(d -> d.equals(answerCandidate)).count();
    ...
    public Map<Category, List<String>> getQuestionGroupedByCategory() {
        ...
        List temp = questions.stream().map(e -> e.category).distinct().collect(Collectors.toList());
        for (var cat : temp
        ) {
            map.put((Category) cat, questions.stream().filter(d -> d.category.equals(cat)).map(e -> e.question).collect(Collectors.toList()));
        }
        return map;
    ...
    public String getLongestLettercountAnwerInAGivenCategory(Category c) {
        return  questions.stream().filter(d->d.getCategory().equals(c)).map(e->e.answers).flatMap(f->f.stream()).reduce("",(l1, l2)-> (l1.length()>l2.length()) ?l1:l2);
